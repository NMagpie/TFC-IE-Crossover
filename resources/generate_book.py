...
def make_book(rm: ResourceManager, i18n: I18n, local_instance: bool = False):
    ...
    book.category('tfc_ie_changes', 'TFC + IE Crossover', 'Changes with TerraFirmaCraft.', 'immersiveengineering:manual', is_sorted=True, entries=(
        entry('ores', 'New Ores', 'tfc_ie_addon:ore/normal_bauxite', pages=(
            text('Bauxite is an ore of $(thing)Aluminum$(), which is an integral part of high-voltage architecture. It can be found at any elevation, but deeper veins are often richer. It can be found in $(l:the_world/geology#sedimentary)Sedimentary$() and $(l:the_world/geology#metamorphic)Metamorphic$() rocks.', title='Bauxite').link(*['tfc_ie_addon:ore/%s_%s' % (g, 'bauxite') for g in GRADES_ALL]).anchor('bauxite'),
            multimultiblock('Bauxite Ores in Marble.', *[block_spotlight('', '', 'tfc_ie_addon:ore/%s_%s/%s' % (g, 'bauxite', 'marble')) for g in GRADES]),
            text('Galena is an ore of $(thing)Lead$(), which is used in creating early accumulators. It can be found at elevations below y=75. It can be found in $(l:the_world/geology#metamorphic)Metamorphic$(), $(l:the_world/geology#igneous_extrusive)Igneous Extrusive$() and $(l:the_world/geology#igneous_intrusive)Igneous Intrusive$() rocks.', title='Galena').link(*['tfc_ie_addon:ore/%s_%s' % (g, 'galena') for g in GRADES_ALL]).anchor('galena'),
            multimultiblock('Galena Ores in Marble.', *[block_spotlight('', '', 'tfc_ie_addon:ore/%s_%s/%s' % (g, 'galena', 'marble')) for g in GRADES]),
            text('Uraninite is an ore of $(thing)Uranium$(). It can be found at elevations below y=75. It can be found in $(l:the_world/geology#metamorphic)Metamorphic$() and $(l:the_world/geology#igneous_extrusive)Igneous Extrusive$() rocks. A uranium block can serve as a heat source for the $(thing)Thermoelectric Generator$().', title='Uraninite').link(*['tfc_ie_addon:ore/%s_%s' % (g, 'uraninite') for g in GRADES_ALL]).anchor('uraninite'),
            multimultiblock('Uraninite Ores in Marble.', *[block_spotlight('', '', 'tfc_ie_addon:ore/%s_%s/%s' % (g, 'uraninite', 'marble')) for g in GRADES])
        )),
